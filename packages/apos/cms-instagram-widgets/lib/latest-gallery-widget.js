...
module.exports = (self) => {
   ...
   async function loadGallery(req, username) {
      ...
      const { profile: fromCache, status } = await self.getCachedResult(username);
      if (fromCache) {
      ...
      if (status === 'ERR') {
         log('loadGallery: %s: restored error prevents loading', username);
         return null;
      }
      ...
      if (error || !profile) {
         ...
         log(
            'success: %s (%s) has %d media items',
            cacheResult(username, error, profile).userName,
            profile.name,
